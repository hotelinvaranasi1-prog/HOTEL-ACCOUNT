...
export default function BookingList() {
  ...
  return (
    <div className="space-y-6">
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-50">
              {isLoading ? (
                ...
                <tr 
                  key={`booking-list-row-${b.id}-${b.date}-${idx}`} 
                  className={cn(
                ...
                >
                  <td className="px-6 py-4 border-b border-slate-50">
                    <div className="flex flex-col">
                      <span className="text-sm font-medium text-slate-600">{b.date}</span>
                      {b.check_in && b.check_out && (
                        <div className="flex items-center gap-1.5 mt-1">
                          ...
                          <span className="text-[9px] font-black bg-indigo-50 text-indigo-600 px-1.5 py-0.5 rounded uppercase tracking-tighter">
                            {Math.max(1, differenceInDays(parseISO(b.check_out), parseISO(b.check_in)))} Nights
                          </span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {b.booking_status === 'Active' && (
                        <button 
                          ...
                          className="p-2 hover:bg-slate-900 hover:text-white rounded-lg transition-all"
                          title="Generate Invoice (Stay)"
                        >
      ...
      <AnimatePresence>
        {quickPayBooking && (
          <div key="quick-pay-modal" className="fixed inset-0 z-[70] flex items-center justify-center p-4">
            <motion.div
              key="quick-pay-overlay"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
      ...
      <AnimatePresence>
        {miscChargeBooking && (
          <div key="booking-misc-modal" className="fixed inset-0 z-[70] flex items-center justify-center p-4">
            <motion.div
              key="booking-misc-overlay"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm"
